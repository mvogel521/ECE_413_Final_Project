...
router.post("/update", function (req,res){
    Student.findOneAndUpdate({ name: req.body.name}, req.body, function(err, doc){
        if (err){
            ...
            let msgStr;
            if (doc == null){
                msgStr = `Student name (name: ${req.body.name}) info does not exist in DB`;
            }
            else{
...
router.post('student/search', function(req, res){
    Student.searchStudent({gpa: {$gt : req.body.gpa}}, function(err, docs){
        if (err){
            let msgStr = 'Something wrong....';
            res.status(201).json({message: msgStr, err: err});
        }
...
router.post('student/read', function(req, res){
    Student.readStudent({name: re})
})
